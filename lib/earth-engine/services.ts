...
export class SoilTemperatureService {
  ...
  async getSoilTemperatureData(query: GEETemperatureQuery): Promise<GEETemperatureResponse> {
    try {
      ...
      const soilTemperatureCollection = era5Collection.select([...ERA5_LAND_CONFIG.temperatureBands])
  ...
  /**
   * Obtiene datos de humedad del suelo para una ubicación y período específicos.
   */
  async getSoilMoistureData(query: GEETemperatureQuery): Promise<GEEMoistureResponse> {
    try {
      const validation = this.validateQuery(query)
      if (!validation.isValid) {
        return {
      ...
      const soilMoistureCollection = era5Collection.select([...ERA5_LAND_CONFIG.moistureBands])

      const extractMoistureData = async (): Promise<GEESoilMoistureData[]> => {
        return new Promise((resolve, reject) => {
          soilMoistureCollection.getRegion(point, ERA5_LAND_CONFIG.scale, 'epsg:4326')
            .evaluate((result: unknown, error: unknown) => {
              if (error) {
                reject(error)
                return
              ...
              try {
                ...
                const headers = data[0] as string[]
                const moistureData: GEESoilMoistureData[] = []

                const timeIndex = headers.indexOf('time')
                const level1Index = headers.indexOf('volumetric_soil_water_layer_1')
                const level2Index = headers.indexOf('volumetric_soil_water_layer_2')
                const level3Index = headers.indexOf('volumetric_soil_water_layer_3')
                const level4Index = headers.indexOf('volumetric_soil_water_layer_4')
                ...
                for (let i = 1; i < data.length; i++) {
                  ...
                  const item: GEESoilMoistureData = {
                    date,
                    moisture_level_1:
                      level1Index >= 0 && row[level1Index] !== null
                        ? Number(row[level1Index])
                        : undefined,
                    moisture_level_2:
                      level2Index >= 0 && row[level2Index] !== null
                        ? Number(row[level2Index])
                        : undefined,
                    moisture_level_3:
                      level3Index >= 0 && row[level3Index] !== null
                        ? Number(row[level3Index])
                        : undefined,
                    moisture_level_4:
                      level4Index >= 0 && row[level4Index] !== null
                        ? Number(row[level4Index])
                        : undefined,
                  }

                  moistureData.push(item)
                }

                moistureData.sort((a, b) => a.date.localeCompare(b.date))
                resolve(moistureData)
              } catch (processError) {
                reject(processError)
              }
            })
      ...
      const moistureData = await extractMoistureData()
      ...
      return {
        ...
        metadata: {
          ...
          recordCount: moistureData.length,
          dataset: ERA5_LAND_CONFIG.dataset,
        },
      }
    } catch (error) {
      console.error('❌ Error obteniendo datos de humedad del suelo:', error)

      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Error desconocido al obtener datos de humedad',
      }
    }
  }
}
